...
public class ResolverImpl implements Resolver {
  ...
  private void configureRemoteRepositories(List<String> remoteMavenRepos, Proxy proxy,
      Proxy secureProxy) {
    int count = 0;
  ...
  public List<Artifact> resolve(Artifact artifact, boolean transitive, List<String> exclusions) {
    ...
    try {
      if (!transitive) {
        ...
        artifactResults = Collections
            .singletonList(system.resolveArtifact(session, artifactRequest));
      } else {
                    ...
                    .map(e -> {
                      ...
                      if (segments.length != 2) {
                        throw new IllegalStateException(
                            "Invalid exclusion format: " + e + " - exclusion are " +
      ...
      throw new IllegalArgumentException(
          "Cannot find module " + artifact.toString() + ". Maybe repository URL is invalid?");
    }
  ...
  protected void customizeRemoteRepoBuilder(RemoteRepository.Builder builder) {
    ...
    if (updatePolicy != null && !updatePolicy.isEmpty()) {
      builder.setSnapshotPolicy(
          new RepositoryPolicy(true, updatePolicy, RepositoryPolicy.CHECKSUM_POLICY_WARN));
    }
  }
